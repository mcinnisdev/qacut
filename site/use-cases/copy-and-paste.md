...
description: "Take a screenshot with one hotkey, drag the region and press Copy. The shot is on your clipboard for a chat, an email or a ticket, and saved on disk too."
---
...
3. Press **Copy**, or `Ctrl+C`. The screenshot is on the clipboard as an image.
4. Paste it into Teams, Slack, an email or a ticket.
5. The window closes. The shot is kept under `~/QACut/Quick/` if you need it again.
...
- **Copy** writes any marks you drew into the file first, so the saved shot matches what you pasted.
- Type a note first and Copy prints it in a band under the picture. **Copy for agent** (`Ctrl+Shift+A`) is the agent version: the path and the note as text.
